...
const Button: React.FunctionComponent<ButtonProps> =
    (
        ...
        let customClassName: string = className += " py-1 px-4 rounded-lg ";
        customClassName += (buttonType === "primary") ? "bg-[#4F46E5] text-white" : (buttonType === "secondary" ? "bg-zinc-100 text-zinc-900" : "");
        return (
            <button {...restProps} className={customClassName}>{isLoading ? <Loading
                stroke={buttonType === "primary" ? "#feffff" : "#000000"} /> : children}</button>
        )
    }
